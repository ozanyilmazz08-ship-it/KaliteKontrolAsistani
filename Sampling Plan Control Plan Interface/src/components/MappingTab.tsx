...
import { Search, Filter, ChevronRight, Info, AlertCircle, CheckCircle } from 'lucide-react';
...
export function MappingTab() {
  ...
  const [processStepFilter, setProcessStepFilter] = useState('all');
  const [criticalityFilter, setCriticalityFilter] = useState('all-crit');
  
  // Form state with validation
  const [lotSizeRange, setLotSizeRange] = useState('281-500');
  const [sampleSize, setSampleSize] = useState('50');
  const [inspectionLevel, setInspectionLevel] = useState('II');
  const [acceptNumber, setAcceptNumber] = useState('1');
  const [rejectNumber, setRejectNumber] = useState('2');
  const [inspectionFrequency, setInspectionFrequency] = useState('every-lot');
  const [applyToProcessStep, setApplyToProcessStep] = useState(false);
  const [overrideCustomer, setOverrideCustomer] = useState(false);
  ...
  const toggleAllRows = () => {
    setSelectedRows(selectedRows.length === filteredCharacteristics.length ? [] : filteredCharacteristics.map(c => c.id));
  };
  ...
  const filteredCharacteristics = characteristics.filter(char => {
    ...
    const matchesProcess = processStepFilter === 'all' || char.processStep.toLowerCase() === processStepFilter;
    const matchesCriticality = criticalityFilter === 'all-crit' || 
                                (criticalityFilter === 'critical' && char.criticality === 'C') ||
                                (criticalityFilter === 'significant' && char.criticality === 'S') ||
                                (criticalityFilter === 'routine' && char.criticality === 'R');
  ...
  const validateField = (field: string, value: string) => {
    ...
    switch (field) {
      case 'lotSizeRange':
        result = validateLotSizeRange(value);
        break;
      case 'sampleSize':
        result = validateSampleSize(value);
        break;
      case 'acceptNumber':
        result = validateAcceptanceNumber(value);
        break;
      case 'rejectNumber':
        result = validateRejectionNumber(value, parseInt(acceptNumber, 10));
        break;
      default:
        return;
    }
  ...
  const handleApplyChanges = () => {
    // Validate all fields
    const validations = {
      lotSizeRange: validateLotSizeRange(lotSizeRange),
      sampleSize: validateSampleSize(sampleSize),
      acceptNumber: validateAcceptanceNumber(acceptNumber),
      rejectNumber: validateRejectionNumber(rejectNumber, parseInt(acceptNumber, 10)),
    };
    ...
    if (hasErrors) {
      toast.error('Please correct validation errors before applying changes');
      return;
    }
    ...
    // Apply changes
    const selectedPlanName = samplingPlans.find(p => p.id === selectedPlan)?.name;
    toast.success(`Applied "${selectedPlanName}" to ${selectedRows.length} characteristic(s)`);
  ...
  const handleFilterClick = () => {
    toast.info('Advanced filter dialog would open here');
  };
    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 border-r flex flex-col">
        ...
        <div className="border-b bg-slate-50 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            <Select defaultValue="all" onValueChange={setProcessStepFilter}>
            ...
            <Select defaultValue="all-crit" onValueChange={setCriticalityFilter}>
              <SelectTrigger className="w-[180px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr className="border-b">
                <th className="text-left p-3 w-12">
                  <Checkbox 
                    checked={selectedRows.length === filteredCharacteristics.length}
            ...
            <>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input 
                        ...
                        className="mt-1"
                        onChange={(e) => {
                          setLotSizeRange(e.target.value);
                          validateField('lotSizeRange', e.target.value);
                        }}
                      />
                      {errors.lotSizeRange && (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input 
                          ...
                          onChange={(e) => {
                            ...
                            validateField('sampleSize', e.target.value);
                          }}
                        />
                        {errors.sampleSize && (
                          <div className="text-red-500 text-sm mt-1">
                      ...
                      <div>
                        ...
                        <Select defaultValue="II" onValueChange={setInspectionLevel}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input 
                          ...
                          className="mt-1"
                          onChange={(e) => {
                            setAcceptNumber(e.target.value);
                            validateField('acceptNumber', e.target.value);
                          }}
                        />
                        {errors.acceptNumber && (
                          <div className="text-red-500 text-sm mt-1">
                      ...
                      <div>
                        ...
                        <Input 
                          ...
                          className="mt-1"
                          onChange={(e) => {
                            setRejectNumber(e.target.value);
                            validateField('rejectNumber', e.target.value);
                          }}
                        />
                        {errors.rejectNumber && (
                          <div className="text-red-500 text-sm mt-1">
                    ...
                    <div>
                      ...
                      <Select defaultValue="every-lot" onValueChange={setInspectionFrequency}>
                        <SelectTrigger className="mt-1">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
              ...
              <Card className="p-4 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={applyToProcessStep} onCheckedChange={setApplyToProcessStep} />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={overrideCustomer} onCheckedChange={setOverrideCustomer} />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={autoEscalate} onCheckedChange={setAutoEscalate} defaultChecked />
                </div>
              </Card>
