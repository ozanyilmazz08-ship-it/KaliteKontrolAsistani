...
export function calculateRisk(forecast: ForecastPoint[], specs: SpecLimits): RiskMetrics {
  // Simplified risk calculation using normal approximation
  ...
  const zLSL = (avgMedian - specs.LSL) / sigma;
  const zUSL = (specs.USL - avgMedian) / sigma;
  
  // Rough normal CDF approximation
  const probLSL = zLSL < 0 ? 0.5 * Math.exp(-0.5 * zLSL * zLSL) / Math.sqrt(2 * Math.PI) : 0.01;
  const probUSL = zUSL < 0 ? 0.5 * Math.exp(-0.5 * zUSL * zUSL) / Math.sqrt(2 * Math.PI) : 0.01;
  
  const totalOutProb = probLSL + probUSL;
  const ppm = totalOutProb * 1e6;
  const yield_ = (1 - totalOutProb) * 100;
  
  return {
    probabilityLSL: probLSL,
    probabilityUSL: probUSL,
    expectedPPM: Math.max(10, Math.min(ppm, 5000)),
    forecastedYield: Math.max(95, yield_),
    zRisk: Math.min(zLSL, zUSL)
  };
}
